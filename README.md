...
- **답변이 빈 문자열로 나오는 케이스** — 답이 없는 질문 하나에서 거절도 답변도 하지 않습니다. 원인 미확인이며, 채점기가 이를 환각으로 세고 있어 지표도 함께 왜곡됩니다.
- **검색 지연 4배 증가** — 클라우드 임베딩 전환으로 64ms → 262ms. 임베딩 API 왕복이 원인이며, 질문 임베딩 캐시가 후보입니다.
- **평가셋 규모** — 18문항으로는 0.02 수준의 차이를 우연과 구분할 수 없습니다.
- **배포** — 임베딩 의존성이 해소되어 진행 중입니다. 남은 것은 H2 → Postgres 전환과 호스팅입니다.
